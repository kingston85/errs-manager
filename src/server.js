...
const { csrfSynchronisedProtection, invalidCsrfTokenError, generateToken } = require('./lib/csrf');
const { attachNotifications } = require('./lib/notifications');
  ...
  helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        // Google Fonts is the one external host allowed in — the stylesheet
        // itself comes from fonts.googleapis.com, the actual font files it
        // references from fonts.gstatic.com (a separate directive; browsers
        // treat @font-face src as a font load, not a style load).
...
// Public license verification (see src/routes/verify.js's header comment)
// — intentionally reachable with no session, same as /login.
...
app.use((err, req, res, next) => {
  if (err === invalidCsrfTokenError || err.code === 'EBADCSRFTOKEN') {
    ...
    // The middleware that normally sets res.locals.csrfToken (server.js,
    // just below csrfSynchronisedProtection) never got to run on a request
    // this middleware rejected — every authenticated page (via the sidebar's
    // sign-out form) reads res.locals.csrfToken, so without this the error
    // page itself throws a ReferenceError instead of explaining what happened.
    if (req.session) res.locals.csrfToken = generateToken(req);

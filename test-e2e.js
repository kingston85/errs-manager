...
(async () => {
  ...
  let documentNumber = null;
  {
    ...
    // New: renewal workflow (src/routes/documents.js's POST /:id/renew).
    // The button itself only renders when a case is within 60 days of
    // expiry (see isRenewable), which this freshly-issued case usually
    // isn't — so this drives the route directly (a real form, built and
    // submitted in-page with a live CSRF token) rather than depending on
    // today's date lining up with the eligibility window.
    let renewedCaseId = null;
    if (issuedCaseId) {
      await page.goto(`${BASE}/app/documents/${issuedCaseId}/edit`);
      const csrf = await page.$eval('input[name="_csrf"]', (el) => el.value);
      await Promise.all([
        page.waitForNavigation(),
        page.evaluate(({ id, csrf }) => {
          const f = document.createElement('form');
          f.method = 'POST';
          f.action = `/app/documents/${id}/renew`;
          const i = document.createElement('input');
          i.type = 'hidden'; i.name = '_csrf'; i.value = csrf;
          f.appendChild(i);
          document.body.appendChild(f);
          f.submit();
        }, { id: issuedCaseId, csrf }),
      ]);
      renewedCaseId = (page.url().match(/\/documents\/(\d+)\/edit/) || [])[1] || null;
      log(!!renewedCaseId && renewedCaseId !== String(issuedCaseId), '[depthead] renew creates a new case with its own id');
      bodyText = await page.textContent('body');
      log(bodyText.includes('This case renews'), '[depthead] renewed case shows linkage back to the source case');
      log(bodyText.includes('Application Received') || bodyText.includes('APPLICATION_RECEIVED') || !bodyText.includes('Issued Document'), '[depthead] renewed case starts fresh (not pre-issued)');

      // And the reverse link: the original case should now show "already renewed as".
      await page.goto(`${BASE}/app/documents/${issuedCaseId}/edit`);
      bodyText = await page.textContent('body');
      log(bodyText.includes('Already renewed as'), '[depthead] source case shows it has been renewed');
    }
    ...
    // not a link/button/form inside it — should navigate to the row's edit
    // page. td:nth-child(2), not (1) — the first cell is now the bulk-select
    // checkbox column, and a click there should NOT navigate (checked below).
    await page.goto(`${BASE}/app/companies`);
    const firstDataHref = await page.$eval('table.data-table tbody tr', (tr) => tr.getAttribute('data-href'));
    await Promise.all([page.waitForNavigation(), page.click('table.data-table tbody tr:first-child td:nth-child(2)')]);
    log(!!firstDataHref && page.url().endsWith(firstDataHref), '[depthead] clicking a table row navigates to its edit page');

    // Bulk select / delete / export (initBulkActions + generic.js's
    // bulk-delete route + ids-filtered CSV export).
    await page.goto(`${BASE}/app/companies`);
    await page.click('table.data-table tbody tr:first-child .bulk-row-check');
    const toolbarVisibleAfterCheck = await page.$eval('#bulk-toolbar', (el) => !el.hidden);
    log(toolbarVisibleAfterCheck, '[depthead] checking a row reveals the bulk-action toolbar');
    const exportHref = await page.$eval('#bulk-export-link', (el) => el.getAttribute('href'));
    log(exportHref.includes('ids='), '[depthead] bulk export link picks up the selected row id');
    const bulkExportResp = await page.request.get(`${BASE}${exportHref}`);
    log(bulkExportResp.status() === 200 && (bulkExportResp.headers()['content-type'] || '').includes('csv'), '[depthead] bulk-selected export responds with csv content-type');

    // Bulk delete: create a disposable company, select just it, delete it.
    // Driven at the request level (real CSRF token, real session cookie)
    // rather than clicking the confirm()-guarded button in-page — this is
    // the same server route the button's JS ultimately submits to, and
    // sidesteps flakiness in stacking multiple browser-native dialogs
    // within one Playwright run.
    await page.goto(`${BASE}/app/companies/new`);
    await page.fill('#f_name', 'Bulk Delete Target Co');
    await Promise.all([page.waitForNavigation(), page.click('button:has-text("Create")')]);
    await page.goto(`${BASE}/app/companies?q=Bulk+Delete+Target`);
    const targetCheckbox = await page.$('table.data-table tbody tr .bulk-row-check');
    if (targetCheckbox) {
      const targetId = await targetCheckbox.getAttribute('value');
      const bulkCsrf = await page.$eval('#bulk-delete-form input[name="_csrf"]', (el) => el.value);
      const bulkDelResp = await page.request.post(`${BASE}/app/companies/bulk-delete`, {
        form: { ids: targetId, _csrf: bulkCsrf },
      });
      await page.goto(`${BASE}/app/companies?q=Bulk+Delete+Target`);
      bodyText = await page.textContent('body');
      log(bulkDelResp.ok() && !bodyText.includes('Bulk Delete Target Co'), '[depthead] bulk delete removed the selected record');
    } else {
      log(false, '[depthead] bulk delete removed the selected record');
    }

    // Global search (src/routes/search.js): finds a company by name from
    // the topbar box, scoped correctly, without needing to know which list
    // page it lives on.
    await page.goto(`${BASE}/app/search?q=Acme`);
    bodyText = await page.textContent('body');
    log(bodyText.includes('Acme Testing Corporation'), '[depthead] global search finds a company by name');
    await page.goto(`${BASE}/app/search?q=zzz-no-such-record-zzz`);
    bodyText = await page.textContent('body');
    log(bodyText.toLowerCase().includes('no records match'), '[depthead] global search reports no results for a made-up query');

...
router.get('/', async (req, res) => {
  ...
  // Issued licenses/certificates expiring within 60 days (or already past
  // expiry) that haven't already had a renewal case started — feeds the
  // "Renew" workflow in src/routes/documents.js. Same 60-day horizon that
  // route uses, so what's flagged here matches what actually offers Renew.
  const expiringSoonQ = db.query(
    `SELECT cd.id, cd.document_number, cd.expiry_date, c.name AS company_name, dt.label AS type_label, un.name AS unit_name, cd.unit_id
     FROM case_documents cd
     JOIN companies c ON c.id = cd.company_id
     JOIN document_types dt ON dt.id = cd.document_type_id
     JOIN units un ON un.id = cd.unit_id
     WHERE cd.status = 'ISSUED' AND cd.expiry_date IS NOT NULL
       ...
       ${unitFilter ? 'AND cd.unit_id = $1' : ''}
     ORDER BY cd.expiry_date ASC LIMIT 8`,
  ...
  const [caseStats, complaintsOpen, reminders, assetsNeedingAttention, kpis, recentActivity, trendMonths, expiringSoon] = await Promise.all([
    caseStatsQ, complaintsOpenQ, remindersQ, assetsNeedingAttentionQ, kpiQ, recentActivityQ, trendMonthsQ, expiringSoonQ,
  ]);

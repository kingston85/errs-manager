...
// A forced-password-reset gate: an account flagged must_change_password
// (every newly created account, every Dept-Head-initiated reset, and every
// originally-seeded demo account — see db/migrations) can't reach anything
// else in the app until they set their own password at /account/password.
function requirePasswordSet(req, res, next) {
  if (req.user && req.user.must_change_password && !req.path.startsWith('/account')) {
    return res.redirect('/account/password');
  }
  next();
}

module.exports = { hashPassword, verifyPassword, attachUser, requireAuth, requireRole, requirePasswordSet };

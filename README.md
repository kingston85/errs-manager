...
- **File attachments** — a scanned application, a site photo, a lab report — attach a file to almost any record. Files are stored in Postgres itself (not the filesystem — see "Notes on the data model"), so they survive redeploys on Render's free tier. Every list view shows a "📎 N" badge on rows that have files attached (hover it to see the filenames), so you can spot which records have documentation without opening each one.
...
`GET /internal/send-due-reminders` emails each Unit Head / Department Head a single daily digest covering (a) their unit's reminders due within 7 days and (b) their unit's issued licenses/certificates expiring within 60 days — then marks both as notified so nobody gets the same item every day (only once *every* recipient's send succeeds — if one fails, everything stays unmarked so the next run retries it). It's inert (returns what it *would* send, sends nothing) unless `RESEND_API_KEY` is set, and it always requires a matching `?token=<INTERNAL_TASK_TOKEN>` query parameter — without `INTERNAL_TASK_TOKEN` set, the route refuses every request.

Mail goes out through [Resend](https://resend.com)'s HTTPS API rather than raw SMTP — Render's free web-service tier blocks all outbound traffic to SMTP ports (25/465/587), so nodemailer-style sending just hangs and times out no matter the credentials. Resend's API is plain HTTPS, which isn't blocked.
